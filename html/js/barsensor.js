/*!
	...
	This is the javascript file for the barometer sensor page. It handles event handling such as button pushes and MQTT updates
	and updates the barometer.html page values.
    ...
    function init() {
        ...
        client.on('message', function(topic, message, packet) {
            if (topic == '/bar/press') {
                ...
                barPress.innerHTML = message;
            }
			if (topic == '/bar/alt') {
                /*if ((Number(message) >= 0) && (Number(message) <= 100)) { //conditional statements can be used to update icons and such
                    luxValue.innerHTML = message;
                } */
                barAlt.innerHTML = message;
            }
			else {}
        })
